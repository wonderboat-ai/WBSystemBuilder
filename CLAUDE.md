...
1. **Split visual em Branch A / Branch B** quando power tap não está no extremo do backbone.
2. **Wireless (Yacht Devices YDWG-02, iKommunicate)** como gateway IP cross-brand.
3. **Tools** — calculadoras avulsas (voltage drop standalone, antenna range, banco de baterias).
4. **Versionamento de catálogo** — campo `version: "AAAA.MM"` + `sourceAuthority: url` por catálogo. Rejeitar dados >12 meses.
5. **Revisar os 16 itens pendentes da auditoria de 2026-08-18** (ver `scripts/audit_2026-08-18_pending_review.json`, já reduzido de 53 pra 16 após 3 rodadas extras de correção/pesquisa) — o que sobrou são principalmente portas menores em famílias MFD (Fusion, GPSMAP 1223, GTB 10 etc.) e itens de fabricante terceiro fora do escopo garmin.com (Actisense/Maretron/Yacht Devices).
