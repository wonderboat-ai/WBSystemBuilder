...
- Catálogo Garmin Marine (150 devices + 22 adapters/cabos = 172 SKUs, cobrindo MFD/Radar/Sonar/Autopilot/VHF/AIS/Network/Sensors/Audio/TrollingMotor/Camera/Antenna), auditado item a item contra garmin.com/en-US em 2026-08-18
...
- Toggle Foto oficial Garmin / Silhueta Wonder BOAT nos cards (172/172 SKUs com foto real, 100% cobertura)
...
6. **Revisar os 53 itens pendentes da auditoria de 2026-08-18** (ver `scripts/audit_2026-08-18_pending_review.json`) — SKUs/portas com confiança insuficiente pra correção automática (ex: ECHOMAP UHD2 54cv/72cv sem SKU real localizado, GPSMAP 8612/8612xsv/8616xsv com identidade incerta, GPS 24xd apontando pra produto errado sem substituto confirmado). Precisa de pesquisa adicional ou confirmação do Lucas antes de alterar.
...
- **2026-08-18**: Banco de fotos completado pra 175/175 (100%) — os 9 gaps restantes resolvidos via CDN direto + pesquisa paralela. Em seguida, auditoria completa do catálogo (175 SKUs) cruzando cada item contra garmin.com/en-US via Workflow paralelo (8 lotes, 23 grupos de categoria) — verificando tanto SKU quanto lista de portas/interfaces contra a spec oficial. Resultado: 131/175 (75%) tinham alguma divergência, muitas graves (SKUs cadastrados apontando pra produtos Garmin totalmente diferentes — relógios fēnix/Venu, GPS de golfe Approach, GPS automotivo de caminhão, telêmetro laser Xero, painel de chaveamento EmpirBus). 78 correções de alta confiança aplicadas nesta sessão (SKU corrigido, porta corrigida, ou ambos — ver `scripts/audit_2026-08-18_corrections_log.json`), incluindo 3 remoções de produtos fantasma (GPSMAP 9012/9016 — série 9000 só existe em 19/22/24/27"; PS22-TH — duplicava PS22-TR). Achados sistêmicos por família: radares GMR xHD3 usam porta nativa BlueNet, não Marine Network; Force Kraken (6 variantes) não tem NMEA 2000, conecta só via WiFi embutido; vários transdutores Panoptix (PS22-TR/PS31/PS51-TH) conectam direto na Garmin Marine Network, não por conector proprietário "SonarConn". Novo tipo de porta `ANT` adicionado ao vocabulário (protocolo wireless proprietário Garmin, usado pelo gWind Wireless). 53 itens permanecem pendentes de revisão por falta de confiança suficiente pra correção automática — ver `scripts/audit_2026-08-18_pending_review.json`. Catálogo passou de 175 pra 172 SKUs (150 devices + 22 adapters) após as remoções; cobertura de fotos permanece 100% (172/172).
...
- `scripts/audit_2026-08-18_corrections_log.json` — as 78 correções de SKU/porta aplicadas na auditoria completa de 2026-08-18 (SKU antigo→novo, mudança de porta, motivo/fonte) + os 3 itens removidos por serem produtos fantasma.
- `scripts/audit_2026-08-18_pending_review.json` — os 53 itens da mesma auditoria que ficaram sem correção automática por falta de confiança suficiente — precisam de revisão do Lucas ou pesquisa adicional antes de alterar o catálogo.
