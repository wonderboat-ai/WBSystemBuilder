...
1. **Wireless (Yacht Devices YDWG-02, iKommunicate)** como gateway IP cross-brand.
2. **Tools** — calculadoras avulsas (voltage drop standalone, antenna range, banco de baterias).
3. **Versionamento de catálogo** — campo `version: "AAAA.MM"` + `sourceAuthority: url` por catálogo. Rejeitar dados >12 meses.
4. **Revisar os 16 itens pendentes da auditoria de 2026-08-18** (ver `scripts/audit_2026-08-18_pending_review.json`, já reduzido de 53 pra 16 após 3 rodadas extras de correção/pesquisa) — o que sobrou são principalmente portas menores em famílias MFD (Fusion, GPSMAP 1223, GTB 10 etc.) e itens de fabricante terceiro fora do escopo garmin.com (Actisense/Maretron/Yacht Devices).
...
- **2026-08-18**: Split visual Branch A/B implementado na vista N2K Backbone (item 1 do roadmap) — quando o power tap não está num extremo do backbone (ou seja, tem dispositivos nos dois lados dele na ordem física), o trunk vertical agora é desenhado como 2 trechos visualmente distintos em vez de uma linha única contínua: cor azul (`#5b8df6`) pro trecho antes do power tap ("BRANCH A"), cor dourada (`#d4a64a`) pro trecho depois ("BRANCH B"), com uma folga visual de 20px acima/abaixo do ícone do power tap marcando o ponto de inserção de energia — reflete o que acontece fisicamente no cabo (a corrente sai em 2 direções opostas a partir da T de alimentação, cada uma terminando no seu próprio terminador 120Ω). Cada ramal ganhou um rótulo com subtotal (quantidade de dispositivos, ΣLEN, comprimento do trecho, tensão mínima) desenhado no canvas (acima/abaixo do trunk) e replicado como 2 linhas extras no painel "N2K Summary" do inspector. Quando o power tap está num extremo (ou não há power tap no projeto), o comportamento é idêntico ao anterior — linha cinza única, sem rótulos — garantindo retrocompatibilidade total com projetos existentes. Implementado via novo helper `n2kBranchSplit(nodes)` (logo após `calcN2kVoltagesPerNode()`), que reaproveita os campos `distance`/`V` já calculados pelas 2 direções do cálculo de voltage drop (nenhuma mudança na fórmula em si) — evita duplicar a lógica entre a vista e o inspector. A view ganhou padding extra no topo/fundo (58px em vez de 40px) só quando há split, pra caber os rótulos sem colidir com a primeira/última linha de dispositivo. Testado via injeção de estado direto no console do browser (8 cenários: split assimétrico 2/3, sem split com power no extremo, e split recalculando ao vivo durante um arrasto simulado que move um nó pro outro lado do power tap) — sem erros de console, SVG bem-formado, sem sobreposição de elementos, painel do inspector consistente com o canvas. O modo de impressão A3 Técnico não foi afetado (a página N2K Backbone do PDF usa `buildSchematicSvg`, não `renderN2kBackboneView` — são caminhos de código separados).
- **2026-08-18**: Drag-to-reorder implementado na vista N2K Backbone (item 1 do roadmap) — cada linha ganhou um handle (⠿) que permite arrastar o dispositivo pra outra posição no backbone, refletindo a ordem física real dos equipamentos a bordo em vez da ordem fixa por categoria. Durante o arrasto a lista reflui em tempo real (preview local, sem mutar `state.project`); ao soltar, grava `n2kOrder` sequencial (0..N-1) em todos os nós do backbone, que já era o campo lido por `buildN2kBackbone()` mas nunca era escrito por nenhum lugar do código (a ordem custom nunca tinha sido implementável até agora). Corrigido de brinde um bug latente nessa mesma função: o guard "tem ordem customizada?" usava `.some(n2kOrder)` (truthy), que ignorava silenciosamente um `n2kOrder:0` legítimo — trocado pra `!==undefined`. Novo helper `svgViewBoxPoint()` complementa o `svgPoint()` existente: converte coordenada de tela pra espaço do viewBox via `getScreenCTM()`, necessário porque a vista N2K Backbone tem viewBox próprio (altura = conteúdo, não o pan/zoom da vista "Tudo") — usar `svgPoint()` ali daria coordenadas erradas. Terminadores e power tap também são arrastáveis (sem trava de posição), consistente com o cálculo de voltage drop já suportar power tap fora do centro. Testado via simulação de eventos de mouse reais no DOM (mousedown/mousemove/mouseup) contra 6 dispositivos injetados — ordem final, clamping nos limites, e recálculo de voltagem todos confirmados corretos.
- **2026-08-18**: Sessão grande — 15 itens reportados pelo Lucas via lista numerada corrigidos de uma vez (levantamento prévio via Workflow paralelo, 10 agentes de pesquisa, antes de qualquer edição), seguido de auditoria final (Workflow, 4 lentes × verificação adversarial) que encontrou e corrigiu mais 9 bugs reais introduzidos pelas próprias mudanças. Resumo por item:
  - **Item 0** — roteamento de edges na vista "Tudo": linhas agora desviam automaticamente de bbox de outros devices no caminho (`findBlockingNode`/`segmentHitsNodeBbox`, teste de cruzamento com as 4 arestas do bbox+margem, sem falso-positivo em endpoint só encostando um vizinho) e suportam ajuste manual — grip arrastável na edge selecionada grava `edge.waypoints[0]`, duplo-clique reseta pro roteamento automático. Path vira curva quadrática (`Q`) através do waypoint quando existe; sem waypoint/colisão, comportamento idêntico ao anterior (bezier cúbico).
  - **Item 1** — regra `R-BN-01` corrigida: reconhece qualquer nó-ponte (device com portas BlueNet+GarminMarineNet conectadas, ex: cabo adaptador 010-12531-01) como resolução válida, não só um device chamado literalmente "BlueNet 30" solto sem conectividade no projeto.
  - **Item 2** — `computePowerUse`/`computePowerRows` ganharam `node.busOverride` ('12V'/'24V', select no inspector quando `dev.power.voltage` é dual) — antes, todo device 12-24VDC contava incondicionalmente no barramento 12V, deixando o total 24V sempre zerado.
  - **Item 3** — J1939 adicionado a GPSMAP 8416xsv/8612/8612xsv/8616/8616xsv (toda a família 8600 estava sem essa porta; confirmado contra o manual de instalação oficial compartilhado 8x10/8x12/8x16).
  - **Item 4** — porta `GPSAntenna` adicionada ao AIS 800 (antena GPS externa opcional, confirmada no manual de instalação oficial, seção "Connection Considerations").
  - **Item 5** — regra `R-J1939-01` corrigida: só dispara se o device J1939 estiver, por qualquer caminho de edges, no mesmo componente conectado que a rede N2K real do projeto — antes disparava pra qualquer device J1939 sem gateway direto, inclusive num barramento CAN 100% isolado (ex: display Yamaha CL5 num barramento próprio sem nenhuma edge tocando o N2K real).
  - **Item 6** — vista Ethernet: cabo hub↔leaf e uplink switch-a-switch ganharam `data-edit-cable` clicável (`_openInlineCableEditor`, mesmo padrão do drop N2K) já que antes não tinha nenhum jeito de editar o comprimento nessa vista. 5 bugs de layout corrigidos: espaço vertical desperdiçado em switch 2+ (groupH usava a lista completa de leaves em vez da fatiada por switch), endpoints agora atribuídos ao switch a que estão de fato cabeados (`switchOf`, antes todos caíam no 1º switch), uplink switch-a-switch passou a ser desenhado (antes só aparecia na Cabling List textual), altura do painel Cabling List agora considera o número de cabos, nomes de modelo longos truncados nos cards.
  - **Item 7** — extensão do backbone N2K via drop-and-insert: soltar um device N2K numa posição Y específica da vista estruturada insere ali (`insertIdx` calculado a partir do cursor), e o fallback de ordenação em `buildN2kBackbone` mudou de `n2kOrder||0` pra `n2kOrder??ORD_END(1e9)` — nó sem ordem cai no fim, não "salta" pro topo por empatar com `n2kOrder:0`.
  - **Item 8** — edição de Item Livre já criado: `state.ui.editingFreeId`, botão ✎ no card da lib e no inspector, formulário pré-popula model/brand/portas e atualiza in-place (id nunca muda).
  - **Item 9/10** — aba Energia virou tabela (`renderPowerTableView`/`computePowerRows`, novo container `#canvas-table-view`) em vez de canvas SVG filtrado: 1 linha por device com A/V/W e sugestão de bitola AWG (`awgForCurrent`, tabela `AWG_TABLE`/`POWER_TARGET_DROP_PCT` em `data.js` — 3% de queda, ABYC E-11 circuito crítico; marcado explicitamente como estimativa de engenharia, não a tabela oficial ABYC certificada). Sem comprimento de cabo cadastrado, mostra "sem comprimento" em vez de inventar.
  - **Item 11** — página N2K BACKBONE do PDF A3 Técnico trocou de `buildSchematicSvg` filtrado pra `buildN2kBackboneSvg` (novo, paleta de cor fixa pra não vazar tema claro/escuro pro papel), reproduzindo a vista estruturada real (T-Joiners, drops, voltagem por nó, split de branch) — **isso substitui a decisão registrada na entrada anterior** ("modo de impressão A3 não foi afetado pelo split de branch"), que valia até esta sessão. Nova página ENERGIA no A3 (`buildEnergiaTableHtml`, mesma tabela da vista on-screen em HTML print-safe).
  - **Item 12/13** — marca d'água do PDF Cliente: removido o texto "WONDER BOAT" duplicado sobre a logo (`.wb-watermark` deletado, só a imagem fica, reposicionada/reduzida pra não invadir o canto do título). Rodapé: selo circular "WONDER BOAT · IN07169" mantido (REGRA #5 exige em todos os PDFs) e a linha do projetista ganhou a logo pequena + nome em negrito — decisão de manter o selo em vez de substituí-lo por completo (pedido original dizia "em vez de"), já que contrariar a regra escrita precisava de confirmação explícita que não veio; sinalizado ao Lucas.
  - **Item 14** — tensão de alimentação do power tap ajustável 12–14.5V (`node.n2kPowerV`, `_openPowerVEditor`, badge clicável na vista N2K) — `calcN2kVoltagesPerNode` usa `baseV` em vez de `12` fixo nas 2 direções do cálculo. Novo helper central `addDeviceNode()` (substitui os pushes diretos no drop do canvas e na busca global) auto-insere o Cabo Alimentação N2K no 1º device N2K do projeto via `maybeAutoInsertN2kPower`.
  - **Auditoria final** (9 bugs confirmados e corrigidos, todos introduzidos pelas mudanças acima): `state.ui.editingFreeId` não era limpo ao trocar/carregar/importar projeto — causava perda silenciosa de um Item Livre novo com toast de falso sucesso (agora limpo em `loadProjectByName`/import/`newProject`, e o save handler cria um item novo em vez de fingir sucesso se o id editado sumiu). Editar as portas de um Item Livre já conectado desalinhava `edge.fromPort/toPort` (índice posicional, não id estável) — agora detecta mudança na composição de portas e remove as edges afetadas com aviso em vez de deixar conexão errada/órfã. Device solto na vista N2K Backbone nascia em `x:0,y:0` (empilhava na origem do canvas ao trocar pra vista Tudo/PDF) — agora herda posição do último node do projeto. `FIX_HANDLERS` e `maybeAutoInsertN2kPower` não atribuíam `n2kOrder` (pego pelo fallback `ORD_END`, iam pro fim absoluto do backbone mesmo depois de um terminador com ordem real) — novo helper `assignSensibleN2kOrder()` insere logo antes do terminador final quando já existe ordem explícita no projeto. `addDeviceNode()` testava zona com o canto do node (já com offset de centralização) em vez do ponto real do cursor — 5º parâmetro `zonePt` opcional adicionado. Rótulos em inglês (BRANCH A/B, Power Tap, T-Joiner, N2K Power Cable) traduzidos pra RAMAL A/B, Alimentação, Conector T, Cabo Alimentação N2K — vazavam pro PDF A3 pela 1ª vez porque o item 11 trocou o caminho de renderização da página N2K. Falso-positivo de colisão no roteamento do item 0: endpoint de uma edge encostando a borda de um vizinho (comum com devices lado a lado) era tratado como bloqueio mesmo sem a linha atravessar o vizinho — removido o teste de containment dos próprios endpoints, mantido só o teste de cruzamento com as arestas do bbox. Ramo morto `filter==='power'` removido de `nodeMatchesFilter`/`filterFn` (inalcançável desde que a vista Energia virou tabela com early-return em `renderCanvas`).
